...
async def handle_order_actions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "confirm_order":
        updated_card = (
            f"{base_text}\n\n"
            f"────────────────────\n"
            f"✅ ORDER/PAYMENT CONFIRMED by {admin_name} at {time_str}"
        )
        try:
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text=(
                    "🌸 *Piece & Petal — ការបញ្ជាទិញត្រូវបានបញ្ជាក់!* 🎉\n\n"
                    "ការបញ្ជាទិញរបស់បងត្រូវបានក្រុមការងារផ្ទៀងផ្ទាត់រួចរាល់ហើយ។ "
                    "យើងខ្ញុំកំពុងរៀបចំវេចខ្ចប់ និងទាក់ទងជូនដំណឹងមុនពេលដឹកជញ្ជូនណា៎ ✨"
                ),
                parse_mode="Markdown"
            )
        except Exception as e:
        ...
        updated_card = (
            f"{base_text}\n\n"
            f"────────────────────\n"
            f"❌ ORDER CANCELLED / REJECTED by {admin_name} at {time_str}"
        )
        try:
            await context.bot.edit_message_text(
                chat_id=msg.chat.id,
        ...
        try:
            await context.bot.send_message(
                chat_id=user_id,
                text=(
                    "⚠️ *Piece & Petal — ការបញ្ជាទិញមានបញ្ហា*\n\n"
                    "សូមអភ័យទោសបង ក្រុមការងារមិនទាន់អាចដំណើរការការបញ្ជាទិញនេះបាននៅឡើយទេ។ "
                    "សូមបងទម្លាក់សារចូលមកក្នុង Chat នេះដើម្បីឱ្យក្រុមការងារជួយសម្របសម្រួលជូនណា៎ 🙏"
                ),
                parse_mode="Markdown"
            )
        except Exception as e:
...
async def checkout(request: Request):
    ...
    payment_method = data.get("payment_method", "KHQR")  # 'KHQR' or 'COD'
    ...
    keyboard = None
    if user_id:
        btn_label = "✅ Confirm Order & Pack" if payment_method == "COD" else "✅ Confirm Payment"
        keyboard = InlineKeyboardMarkup([
            [
                InlineKeyboardButton(btn_label, callback_data=f"confirm_order:{user_id}"),
                InlineKeyboardButton("❌ Reject / Cancel", callback_data=f"reject_order:{user_id}")
            ]
        ])

    method_tag = "💵 CASH ON DELIVERY (COD)" if payment_method == "COD" else "💳 KHQR / BANK TRANSFER"

    order_card = (
        f"🛍️ *NEW IN-APP ORDER SUBMITTED*\n"
        f"────────────────────\n"
        f"• Payment Method: *{method_tag}*\n"
        ...
        f"*Items:*\n{order_summary}\n"
        f"────────────────────\n"
        f"💬 Tap below to confirm or reject:"
    )
    ...
    try:
        if thread_id:
            ...
            # Send targeted message based on payment method
            if payment_method == "COD":
                await tg_app.bot.send_message(
                    chat_id=user_id,
                    text=(
                        f"🌸 *Piece & Petal Order Received!*\n\n"
                        f"អរគុណបង {customer_name}! ក្រុមការងារបានទទួលការបញ្ជាទិញតម្លៃ *${total:.2f}* (ទូទាត់ប្រាក់ពេលទំនិញទៅដល់) រួចរាល់ហើយ។\n\n"
                        f"យើងខ្ញុំកំពុងរៀបចំឥវ៉ាន់ និងទាក់ទងបញ្ជាក់ជូនបងមុនពេលដឹកជញ្ជូនណា៎ 🚚✨"
                    ),
                    parse_mode="Markdown"
                )
            else:
